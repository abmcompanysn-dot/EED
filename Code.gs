...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'getComments': result = getComments(e.parameter); break;
...
function handlePrayerRequest(payload) {
  try {
    ...
    const headers = ['Timestamp', 'Nom', 'Email', 'Pays', 'Nationalite', 'Telephone', 'Demande', 'Confidentialite'];
 ...
 * @param {Object} params - Les paramètres de la requête GET (doit contenir `articleId`).
 * @returns {Object} Un objet contenant les commentaires.
 */
function getComments(params) {
  try {
    let sheetName, idColumnName, idValue;

    if (params.articleId) {
      sheetName = 'Blog_Commentaires';
      idColumnName = 'ID_Article';
      idValue = params.articleId;
    } else {
      throw new Error("ID de besoin ou d'article manquant.");
    }

    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(sheetName);
    if (!sheet) throw new Error(`La feuille '${sheetName}' est introuvable.`);

    const data = sheet.getDataRange().getValues();
    const headers = data.shift();
    if (!headers || headers.length === 0) return { success: true, comments: [] }; // Feuille vide
    ...
    const comments = data
      .filter(row => String(row[idIndex]) === String(idValue) && row[statusIndex] === 'Approuvé') // Filtre par ID et statut
      .map(row => ({
